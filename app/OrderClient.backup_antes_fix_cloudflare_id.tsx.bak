...
export default function OrderClient({ data }: { data: Data }) {
  ...
  const [todayHoursText, setTodayHoursText] = useState('Cargando horario...')
  ...
  useEffect(() => {
    async function loadTodayHours() {
      try {
        ...
        if (!url || !key) {
          setTodayHoursText('Horario no disponible')
          return
        }
        ...
        if (error || !rows || rows.length === 0) {
          setTodayHoursText('Horario no cargado')
          return
        }
        ...
        if (row.open_time_1 && row.close_time_1) {
          parts.push(`${fmt(row.open_time_1)} a ${fmt(row.close_time_1)}`)
        }

        if (row.open_time_2 && row.close_time_2) {
          parts.push(`${fmt(row.open_time_2)} a ${fmt(row.close_time_2)}`)
        }

        setTodayHoursText(
          parts.length > 0 ? `Hoy: ${parts.join(' · ')}` : 'Horario no cargado'
        )
      } catch {
        setTodayHoursText('Horario no disponible')
      }
  ...
  useEffect(() => {
    ...
    async function loadStoreStatus() {
      try {
        ...
        const { data: demoResponse } = await supabase.rpc(
          'get_public_demo_status_v1',
          { p_slug: data.business.slug }
        )

        if (!cancelled && demoResponse?.ok && demoResponse?.paused) {
