...
export default function PanelClient({
  ...
  async function loadBranding() {
    ...
    if (error) {
      setBrandingMessage(error.message)
      return
    }
    ...
    setBranding({
      ...
      accent_color: data?.accent_color || '#f2b705',
  ...
  async function saveBranding() {
    ...
    if (brandingLogoFile) {
      if (brandingLogoFile.size > 5 * 1024 * 1024) {
        setBrandingMessage('El logo no puede superar 5 MB.')
        return
      }
      ...
      const objectPath = `${businessId}/logo.${safeExt}`

      const { error: uploadError } = await supabase.storage
        .from('business-branding')
        .upload(objectPath, brandingLogoFile, {
          upsert: true,
          cacheControl: '3600',
          contentType: brandingLogoFile.type || undefined,
        })

      if (uploadError) {
        setBrandingMessage(uploadError.message)
        return
      }
    ...
    setBrandingLogoFile(null)
    setBrandingMessage('Identidad visual guardada.')
  }
      ...
      {brandingOpen ? (
        ...
        >
          ...
          >
            ...
            {brandingMessage ? <p>{brandingMessage}</p> : null}
            ...
            <label>
              ...
              <input
                ...
                onChange={e =>
                  setBrandingLogoFile(e.target.files?.[0] || null)
                }
            ...
            >
              <label>
                ...
                <input
                  ...
                  style={{ width: '100%', height: 44, marginTop: 5 }}
                />
              </label>
